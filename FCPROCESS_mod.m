...
for i=1:numdatas
    ...
    QC(i).sessdir_out=sprintf('%sses-%d/func/',QC(i).subdir_out,QC(i).session);
...
%% WRITE OUT MASKS FOR NUISANCE REGRESSION
for i=1   %%% All masks come out the same
    for j=1:size(QC(i).runs,1)
        wholebrainmask = QC(i).GLMMASK;
        whitemattermask = QC(i).WMMASK;
        cerebspinflmask = QC(i).CSFMASK;
        stupidhack = strrep(QC(i).sessdir_out,'preproc_FCProc',[QC(i).dataFolder '/fmriprep']);
        wholebrainmasktemplate = load_untouch_nii([stupidhack '/' QC(i).naming_str{j} '_space-MNI152NLin6Asym_' res '_desc-preproc_bold.nii.gz']);
        whitemattermasktemplate = wholebrainmasktemplate;
        cerebspinflmasktemplate = wholebrainmasktemplate;
        imgdims = [91 109 91];
        wholebrainmasktemplate.img = reshape(wholebrainmask,imgdims);
        wholebrainmasktemplate.hdr.dime.dim(5) = 1;
        whitemattermasktemplate.img = reshape(whitemattermask,imgdims);
        whitemattermasktemplate.hdr.dime.dim(5) = 1;
        cerebspinflmasktemplate.img = reshape(cerebspinflmask,imgdims);
        cerebspinflmasktemplate.hdr.dime.dim(5) = 1;
        imgnameWB = [stupidhack '/' QC(i).naming_str{1} '_space-MNI152NLin6Asym' res '_desc-WholeBrain_Mask_FCProcess.nii.gz'];  % Get path to bold data pre-task proc
        imgnameWM = [stupidhack '/' QC(i).naming_str{1} '_space-MNI152NLin6Asym' res '_desc-WM_Mask_FCProcess.nii.gz'];  % Get path to bold data pre-task proc
        imgnameCSF = [stupidhack '/' QC(i).naming_str{1} '_space-MNI152NLin6Asym' res '_desc-CSF_Mask_FCProcess.nii.gz'];
        save_untouch_nii(wholebrainmasktemplate, imgnameWB);
        save_untouch_nii(whitemattermasktemplate, imgnameWM);
        save_untouch_nii(cerebspinflmasktemplate, imgnameCSF);
    end
end
...
for i=1:numdatas
    for j=1:size(QC(i).runs,1)
        ...
        %% DP: added this line to make my arrays equal size
        %mvm{i,j}(:,7) = [];
        ...
        switch switches.regressiontype
            case {0,1}
                if switches.GS
                    ...
                    sig=mean(tempimg(QC(i).GLMMASK),1)';
