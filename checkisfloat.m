...
for s = 1:length(ses)
    for r = 1:runs(s)
        ...
        nifti = load_nii(filename);
        if nifti.hdr.dime.scl_slope==1 && nifti.hdr.dime.scl_inter == 0
            disp(sprintf('%s is data type float', filename))
        else
            disp(sprintf('IMPORTANT!!!! %s is data type NOT float', filename))
        end
    end
end